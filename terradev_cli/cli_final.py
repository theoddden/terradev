#!/usr/bin/env python3
"""
Terradev CLI Final - Complete Implementation with Automatic Credential Detection
Seamless credential management with automatic detection and setup prompts
"""
...
def cli(ctx, config, verbose):
    ...
    _auto_check_setup(ctx)


def _auto_check_setup(ctx):
    """Automatically check if setup is needed and guide user"""
    auth = ctx.obj["auth"]
    ...
    if not providers:
        ...
        print("It looks like you haven't set up any cloud providers yet.")
        print()

        if click.confirm("Would you like to set up cloud providers now?"):
            _run_setup_wizard(ctx)
        else:
            print("💡 You can set up providers later by running:")
            print("   terradev setup")
            print("   terradev configure --provider <provider>")
            print()
            print("📊 Available providers:")
            _show_available_providers()
    else:
        # Check if configured providers have credentials
        missing_creds = []
        for provider in providers:
            creds = auth.get_credentials(provider)
            if not creds or not any(creds.values()):
                missing_creds.append(provider)

        if missing_creds:
            print(f"⚠️  Found {len(missing_creds)} provider(s) without credentials:")
            for provider in missing_creds:
                print(f"   - {provider}")
            print()

            if click.confirm(
                f"Would you like to configure {', '.join(missing_creds)} now?"
            ):
                for provider in missing_creds:
                    _configure_provider_interactive(ctx, provider)


def _show_available_providers():
    """Show available providers with descriptions"""
    providers = {
        "runpod": {
            "name": "RunPod",
            "description": "Specialized GPU cloud (60-70% savings)",
            "difficulty": "Easy",
        },
        "vastai": {
            "name": "VastAI",
            "description": "GPU marketplace (40-60% savings)",
            "difficulty": "Easy",
        },
        "huggingface": {
            "name": "HuggingFace",
            "description": "ML Model Hub (50-80% savings)",
            "difficulty": "Easy",
        },
        "aws": {
            "name": "Amazon Web Services",
            "description": "Largest cloud provider (15-25% savings)",
            "difficulty": "Medium",
        },
        "lambda_labs": {
            "name": "Lambda Labs",
            "description": "GPU cloud for ML (30-50% savings)",
            "difficulty": "Easy",
        },
        "coreweave": {
            "name": "CoreWeave",
            "description": "Specialized GPU cloud (20-40% savings)",
            "difficulty": "Medium",
        },
    }

    for provider, info in providers.items():
        print(f"   {provider:12} - {info['description']} ({info['difficulty']})")
...
@click.pass_context
def setup(ctx, guided):
    """Interactive setup wizard for new users"""
    if guided:
        _run_setup_wizard(ctx)
    else:
        _run_quick_setup(ctx)


def _run_setup_wizard(ctx):
...
def _configure_provider_interactive(ctx, provider: str):
        ...
        if cred == "api_token" and provider == "huggingface":
            ...
            if not value.startswith("hf_"):
                ...
                if not click.confirm("Continue anyway?"):
                    return _configure_provider_interactive(ctx, provider)
        else:
            value = click.prompt(
                f"Enter {cred.replace('_', ' ').title()}",
                hide_input=True,
    ...
    config_obj.save(ctx.obj["config_path"])

    format_success(f"✅ {provider} configured successfully")


def _validate_all_credentials(ctx):
...
def quote(ctx, providers, parallel, gpu_type, region):
    ...
    if not auth.has_credentials():
        print("❌ No cloud provider credentials configured.")
        if click.confirm("Would you like to set up providers now?"):
            _run_setup_wizard(ctx)
            return
        else:
            print("💡 Run 'terradev setup' to configure providers.")
            return
...
@click.option("--providers", "-p", multiple=True, help="Specific providers to use")
@click.option("--parallel", default=6, help="Number of parallel queries")
@click.option("--dry-run", is_flag=True, help="Dry run (no actual provisioning)")
@click.pass_context
def provision(ctx, gpu_type, count, max_price, region, providers, parallel, dry_run):
    ...
    format_success(f"🚀 Starting parallel provisioning for {gpu_type} x{count}")

    # Initialize Terradev engine
    engine = TerradevEngine(config, auth)

    # Run parallel provisioning
    try:
        results = asyncio.run(
            engine.provision_instances(
                gpu_type=gpu_type,
                count=count,
                max_price=max_price,
                region=region,
        ...
        # Display results
        _display_provisioning_results(results, dry_run)

    except Exception as e:
        format_error(f"❌ Provisioning failed: {str(e)}")
        if ctx.obj["verbose"]:
            import traceback
...
def status(ctx, format):
    ...
    try:
        instances = asyncio.run(engine.get_all_instances())
        _display_instances_status(instances, format)

    except Exception as e:
        format_error(f"❌ Status retrieval failed: {str(e)}")
...
def _display_provisioning_results(results, dry_run):
    """Display provisioning results"""
    if dry_run:
        format_warning("🔍 DRY RUN RESULTS")

    format_success(f"✅ Found {len(results['instances'])} optimal instances")
    ...
    for instance in results["instances"]:
        rows.append(
            [
                ...
                instance["gpu_type"],
                f"${instance['price_per_hour']:.4f}",
                instance["region"],
                f"{instance['latency_ms']:.1f}ms",
                f"{instance['optimization_score']:.2f}",
            ]
        )
    ...
    if results.get("cost_analysis"):
        analysis = results["cost_analysis"]
        format_success(f"💰 Cost Analysis:")
        print(f"   Total Cost/Hour: ${analysis['total_cost_per_hour']:.4f}")
        print(f"   Estimated Savings: {analysis['estimated_savings_percent']:.1f}%")
        print(f"   Monthly Savings: ${analysis['monthly_savings']:.2f}")


def _display_instances_status(instances, format):
    """Display instances status"""
    if format == "json":
        print(format_json(instances))
    else:
        format_success(f"✅ Found {len(instances)} instances")

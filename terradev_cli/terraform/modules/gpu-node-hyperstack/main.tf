# Hyperstack GPU Node Module

locals {
  node_name = "${var.cluster_name}-hyperstack-${var.node_config.index}"
  gpu_labels = merge(var.gpu_labels, {
    "terradev.io/provider" = "hyperstack"
    "terradev.io/node-type" = "gpu"
    "terradev.io/created-by" = "terraform"
  })
}

# Create Hyperstack instance
resource "hyperstack_instance" "gpu_node" {
...
resource "null_resource" "wait_for_instance" {
  depends_on = [hyperstack_instance.gpu_node]
  
  provisioner "remote-exec" {
    connection {
      host        = hyperstack_instance.gpu_node.public_ip
      user        = "root"
...
resource "kubernetes_node" "gpu_node" {
  ...
  spec {
    provider_id = hyperstack_instance.gpu_node.id

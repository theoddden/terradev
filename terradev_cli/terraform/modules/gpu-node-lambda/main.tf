# Lambda Labs GPU Node Module

locals {
  node_name = "${var.cluster_name}-lambda-${var.node_config.index}"
  gpu_labels = merge(var.gpu_labels, {
    "terradev.io/provider" = "lambda"
...
resource "lambda_instance" "gpu_node" {
...
resource "null_resource" "wait_for_instance" {
  depends_on = [lambda_instance.gpu_node]
  
  provisioner "remote-exec" {
    connection {
      host        = lambda_instance.gpu_node.public_ip
      user        = "root"
...
resource "kubernetes_node" "gpu_node" {
  ...
  spec {
    provider_id = lambda_instance.gpu_node.id
